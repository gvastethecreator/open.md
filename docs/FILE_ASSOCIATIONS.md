...
OpenMD registers `.md`, `.markdown`, and `.txt` as viewer associations in packaged builds. Registration makes OpenMD available in the operating system's **Open with** UI; it does not silently replace the user's default application.
...
| macOS | Launch Services sends file URLs through Tauri's `RunEvent::Opened`; OpenMD queues them until the webview is ready. | The same event path opens each document without replacing the current window. |
...
## Choosing OpenMD as the default
...
- **macOS:** Finder → Get Info → Open with → OpenMD → Change All.
...
- Keep the bundle role as `Viewer`: OpenMD reads documents and never claims to edit them.
